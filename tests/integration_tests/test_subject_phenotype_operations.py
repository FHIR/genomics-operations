...
def test_find_subject_tx_implications_subject_supplied(client):
    url = tu.find_subject_tx_implications_query('testDateRange=ge2010-01-01&includeVariants=true')
...
def test_find_subject_tx_implications_2(client):
    url = tu.find_subject_tx_implications_query('subject=NB6TK328&treatments=http://www.nlm.nih.gov/research/umls/rxnorm|704')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_TX_IMPLICATIONS_OUTPUT_DIR}2.json', response.json)


def test_find_subject_tx_implications_3(client):
    url = tu.find_subject_tx_implications_query('subject=CA12345&treatments=http://www.nlm.nih.gov/research/umls/rxnorm|1147220')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_TX_IMPLICATIONS_OUTPUT_DIR}3.json', response.json)


def test_find_subject_tx_implications_4(client):
    url = tu.find_subject_tx_implications_query('subject=NB6TK328&haplotypes=CYP2C19 *1/*17,CYP2D6 *1/*41')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_TX_IMPLICATIONS_OUTPUT_DIR}4.json', response.json)
...
def test_find_subject_dx_implications_subject_supplied(client):
    url = tu.find_subject_dx_implications_query('testDateRange=ge2010-01-01&includeVariants=true')
    response = client.get(url)
...
def test_find_subject_dx_implications_1(client):
    url = tu.find_subject_dx_implications_query('subject=huC30902&variants=NC_000001.10:161333381:C:T')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_DX_IMPLICATIONS_OUTPUT_DIR}1.json', response.json)


def test_find_subject_dx_implications_2(client):
    url = tu.find_subject_dx_implications_query('subject=huC30902&conditions=https://www.ncbi.nlm.nih.gov/medgen|C1708353')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_DX_IMPLICATIONS_OUTPUT_DIR}2.json', response.json)


def test_find_subject_dx_implications_3(client):
    url = tu.find_subject_dx_implications_query(
        'subject=NB6TK329&conditions=https://www.ncbi.nlm.nih.gov/medgen|C0677776,https://www.ncbi.nlm.nih.gov/medgen|C4552100,https://www.ncbi.nlm.nih.gov/medgen|C0020445')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_DX_IMPLICATIONS_OUTPUT_DIR}3.json', response.json)


def test_find_subject_dx_implications_4(client):
    url = tu.find_subject_dx_implications_query('subject=HG02657&conditions=https://www.ncbi.nlm.nih.gov/medgen|C3469186')
    response = client.get(url)

    tu.compare_actual_and_expected_output(f'{tu.FIND_SUBJECT_DX_IMPLICATIONS_OUTPUT_DIR}4.json', response.json)
